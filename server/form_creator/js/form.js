 ...
 */
export function getPreloader(text) {
    return `
 ...
 */
export function getModalPreloader(text, footer = "") {
...
$(function () {
    ...
    document.getElementById('__newfile_input').onchange = async function () {
        ...
        changeLoadedForm(content, this.files[0].name.split('.json')[0]);
        info.innerText = "";
    };
...
export function changeLoadedForm(model, id) {
    // Stockage
    loaded_form = model;
