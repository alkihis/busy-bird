...
import { Schema } from "./elements.js";
...
export async function loadFromServer() {
    ...
    const model_list: {
        [formName: string]: [string, boolean]
    } | Error = await User.req("schemas/available.json").then(r => r.ok ? r.json() : new Error(r as any));

    if (model_list instanceof Error) {
        const json = await (model_list.message as any).text();
    ...
    const access_fn = async function(this: HTMLElement) {
        ...
        changeLoadedForm(await resp.json() as Schema, this.dataset.id);
