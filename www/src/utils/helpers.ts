...
export function errorMessage(error_code: number) : string {
    switch (error_code) {
        case 1:
        case 3:
 ...
 */
export function formattedError(error: ErrorObject, additionnal_message?: string) : string {
    return (additionnal_message ? `
        <p class="flow-text no-margin-bottom">${additionnal_message}</p>
    ` : '') + `
        <p class="flow-text error-message ${additionnal_message ? "no-margin-top" : ""}">${errorMessage(error.error_code)}.</p>
